...
- **🚩 Wave 2 BUILT BEHIND OPT-IN FLAGS (SW-106) — full-JSX rewrite of 6 self-contained interactive views, default-OFF (zero live risk):** `formula`, `stock_take`, `egg`, `documents`, `pipeline`, `marketing_automation`. Each adds a THIRD render path (full real-JSX, prop-fed from a chunk-built `data` payload) behind a NEW flag that defaults OFF; when off the existing scaffold path is byte-identical, so **live behavior is unchanged**. Built by a 6-agent parallel workflow + 6 adversarial verifiers — all 6 verified **off-path-safe**. **NOT yet click-tested** (the whole point of shipping OFF) → must verify each on live before promoting.
  - **Flags (append to live URL):** `?react_fp_jsx=1` (formula) · `?react_st_jsx=1` (stock_take) · `?react_egg_jsx=1` (egg) · `?react_dms_jsx=1` (documents) · `?react_pipeline_jsx=1` (pipeline) · `?react_mktauto_jsx=1` (marketing_automation). Each also works via `localStorage crm_<param>='1'`.
  - **Known ON-path caveats to click-test before promoting:** `stock_take` opens default tab (not last-used) + uncontrolled inputs reset after refresh; `egg` config "resync status" writes into a JSX node (status-only) + uncontrolled inputs; `documents`/`egg` use the early-return-before-`useEffect` pattern (safe since `data` is stable per mount); `pipeline` ON-path torn-DOM + 4s stall were **fixed** (chunk early-returns before STEP-2 by-id fills; `PipelineFullJsx` fires `onReady` from its own `useEffect`). Verify PO-save / stock-count / pipeline-stage-move / egg-purchase data paths specifically.
  - **To promote a verified view:** flip its `_react<X>JsxOn()` default to opt-OUT (mirror Wave-1's promoted flags), rebuild, redeploy.
