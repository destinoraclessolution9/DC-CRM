...
- **fude "Redeem Points" → DB-backed request queue** — currently an honest **copy-to-leader** flow (SW-103, no false-success). **➡️ Migration now PREPARED + ready (2026-06-17): `migrations/redemption_requests_2026-06-17.sql`** (table + RLS, idempotent, matches the score_history convention). **YOUR step:** paste it into Supabase dashboard → SQL Editor → Run (project `remuwhxvzkzjtgbzqjaa`) — the PAT-on-CLI security rule blocks me from applying it. Once applied, the client (`confirmRedeemPoints`) gets wired to do a real verified server INSERT with the copy-to-leader path kept as the offline/failure fallback (no false "submitted" success). Client wiring is deliberately deferred until the table exists, because `AppDataStore.create` false-succeeds into localStorage on a missing table.
