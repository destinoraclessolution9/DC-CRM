...
- [x] 3.3 Extract god-functions (#36) — DONE + DEPLOYED (0a89230, Ready). 3 PARALLEL agents (one per file) extracted 4 god-fns into internal helpers; adversarially verified 4/4 equivalent + gate + smoke 9/10. saveActivity (-4 post-save helpers; persist region with early-returns deliberately NOT extracted), renderProspectsTable (3 builders), showProspectDetail (316→75 + 2 builders), showPipelineView (441→257 + 5 builders). Removed a 15-line inert dead comment. ⚠️ handling user WIP: stashed entity-search-cache + mobile-tiles during, restored after (clean pop — different file regions).
