...
- 2026-06-17: Wave 3.1b (router dispatch → _VIEW_RENDER) deployed (da7b6b1, Ready). Used a Python anchor-based replace (file is CRLF — `\n` markers fail). Verified 4 ways incl. 6-agent adversarial workflow (41/41 equivalent). VIEW-REGISTRY CORE DONE (both dispatchers declarative). 7 prod deploys this session, all Ready. NEXT = 3.1c (merge the 2 maps + titles + authz tables into one VIEWS object) — optional polish; the #39 dispatcher-duplication is already gone.
