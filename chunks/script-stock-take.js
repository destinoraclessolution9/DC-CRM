    ...
    const stV2EditExpected = async (shelfId) => {
        ...
        const rowsHtml = expected.map(e => `
            <tr style="border-top:1px solid var(--gray-100);">
                ...
                <td style="padding:6px;text-align:right;"><button class="btn small" style="color:#dc2626;" onclick="(async () => { const sb=window.supabase; if(!sb) return UI.toast.error('Supabase not available'); await sb.from('st_shelf_expected').delete().eq('shelf_id','${_stAttr(shelfId)}').eq('sku','${_stAttr(e.sku)}'); app.stV2EditExpected('${_stAttr(shelfId)}'); })()"><i class="fas fa-trash"></i></button></td>
            </tr>`).join('');
