...
2. Modals/overlays/embedded/mixed (the "modals deferred" category):
   ...
   - [x] **search** (overlay drawer, showSearchPanel) — DONE + PROMOTED (SW-87, 2026-06-16). SearchPanelView.jsx scaffold-shell: React renders overlay+panel chrome (presets, 9-entity select, date inputs, condition-builder chrome, action btns) + stable-id containers (#filter-sections/#extra-filter-sections/#condition-groups/#saved-searches-list/#search-results/#search-pagination) filled by chunk after onReady. Mounted into a host div (overlay/panel are position:fixed so wrapping is safe). hideSearchPanel unmounts island + removes host AND legacy overlay/panel ids (defensive dual cleanup). LIVE parity-identical + fully interactive (entity onChange re-renders, executeSearch 12 rows, presets switch entity, hide/reopen clean no-dupes). Same latent getAll(prospects) cold-crash hit on executeSearch (legacy too).
   - [x] **All 9 modal-content forms via generic passthrough island** (SW-88 opt-in → SW-89/90 promoted default-on, 2026-06-16). New `src/react/views/ModalContentIsland.jsx` renders a chunk-built modal body via `dangerouslySetInnerHTML`; `mountModalContent` uses **flushSync** for synchronous commit so post-render wiring (signature pads, listeners) finds the DOM. Inline onclick/onchange + `<style>` blocks survive (innerHTML honors them). Chunk keeps 100% of logic. **script-fude.js (8):** cps-analysis (21 fields + 2 sig pads + 18 bagua cells), apu-appraisal (27 Likert radios + 3 referral rows + 3 sig pads), destiny-blueprint (37 fields + 2 sig pads + year-label listener), customer-survey (17 radios + sig), story-detail, highlight CRUD (file upload), reward CRUD, fude-redeem — via `_rxShowModal`/`_reactFudeModalsOn`. **script-knowledge.js (1):** quick-capture modal via `_rxShowModal`/`_reactKbModalsOn`. ALL live-verified through the React root with full post-render wiring intact.
   - [ ] **RESIDUAL — genuine rewrites (re-render-own-container-via-innerHTML class):** journey (renderJourneyTab + _refreshJourneyTab), fude showFudeView (412-line monolithic + carousel IIFE + story-filter classList), knowledge kb-slot editors (full-capture/daily-notes/detail — re-render kb-slot on nav + autosave/link wiring). These need section-extraction → stable-id-container refactors (not passthrough — would conflict with their own innerHTML re-renders). Dedicated effort.
...
- [x] referrals (#XL) — SW-78, 2026-06-16, PROMOTED default-on. Scaffold-shell + useEffect-ready; chunk keeps D3 zoom/pan tree + search + filters + style-inject + summary/leaderboard fills. Parity-identical to legacy (summary 1398 / leaderboard 29606 / D3 svg path identical).
- [x] calendar + month (#43/#44) — SW-76, 2026-06-16, PROMOTED default-on (highest-risk). Largest scaffold-shell + useEffect-ready await; chunk renderCalendar/renderTodayActivities/secondary fills the grid + widgets by id. Desktop showCalendarView only (mobile showMobileCalendarView untouched); 'month' dispatches to the same fn so it's covered. Parity-verified opt-in: grid 42 cells + month title + day headers + today activities 1 + birthdays 1 + glance counts (activity 2/followup 2/bday 0). Note: secondary panels (today/glance/bdays) fill on a slower deferred schedule (same as legacy) — grid is the reliable critical path.
- [x] marketing_automation (#32) — SW-74, 2026-06-16, PROMOTED default-on. Scaffold-shell + useEffect-ready await → chunk renderMarketingTabContent fills #marketing-tab-content. Parity-identical to legacy (8 tabs, 4 header btns, content 5070=5070). Reproduced legacy duplicate Monthly-Promotions tab (pre-existing dup — flag for cleanup).
