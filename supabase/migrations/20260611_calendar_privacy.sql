-- ════════════════════════════════════════════════════════════════════
-- Calendar privacy fix — 2026-06-11
-- ────────────────────────────────────────────────────────────────────
-- Problem:
--   get_calendar_window returns prospect_name / customer_name for ALL
--   activities matched by the visibility IN ('open','public') arm,
--   including activities owned by other agents. Any authenticated user
--   could inspect the network response and see client names that do not
--   belong to them.
--
-- Fix:
--   Gate prospect_name / customer_name on ownership. An agent is the
--   "owner" of an activity if:
--     • they are the lead_agent (a.lead_agent_id = p_user_id), OR
--     • they are in the lead's visible subtree
--       (a.lead_agent_id = ANY(p_visible_ids)), OR
--     • they are a co-agent on the activity, OR
--     • p_is_admin = true
--   For public activities viewed by non-owners, both name columns return
--   NULL. The calendar tile still shows the activity (event title / type)
--   — only the client identity is withheld.
--
