...
class DataStore {
    constructor() {
        ...
        this._lightSelects = {
            ...
            //   have rolled over.
            // 2026-07-29: life_chart_type / marital_status / children added.
            //   They were missing here, so EVERY cached prospect row came back
            //   without those keys — the Personal tab rendered both "Use for
            //   life chart" ticks empty, Marital Status "-" and Children 0 no
            //   matter what was stored, and re-saving the edit form wrote the
            //   blank state back over a real value. The intake form has always
            //   written them correctly; the read path dropped them. Any column
            //   the profile/edit screens READ must be listed here, not just the
            //   ones the list view renders.
            // 2026-07-29: the seven "Edit Potential & Opportunities" columns
            //   (estimated_value_min/max, decision_timeline, pain_points,
            //   interests, decision_maker, budget_range) added alongside their
            //   migration (potential_opportunity_columns_2026-07-29.sql). Adding
            //   the DDL without listing them here would have reproduced the exact
            //   bug the migration fixes — stored server-side, invisible client-side.
            prospects: 'id,full_name,nickname,title,gender,nationality,phone,email,ic_number,date_of_birth,lunar_birth,life_chart_type,marital_status,children,ming_gua,element,occupation,company_name,income_range,address,city,state,postal_code,responsible_agent_id,cps_agent_id,cps_assignment_date,protection_deadline,pipeline_stage,deal_value,expected_close_date,status,referred_by,referred_by_id,referred_by_type,referral_relationship,cps_invitation_method,cps_invitation_details,cps_attachment,score,tags,notes,created_at,updated_at,cps_form_date,cps_form_name,cps_form_url,closing_record,closing_records_history,conversion_status,conversion_requested_by,conversion_rejected_by,conversion_rejected_at,closed_at,closed_date,closing_date,potential_level,close_probability,is_own_business,business_name,business_industry,business_area,business_title_role,business_started,company_size,pre2025_purchases,original_source,source_id,source,lead_agent_id,cps_interest,manual_grade,feng_shui_audits,last_activity_date,unable_to_serve,unable_reason,revived_at,revive_notes,apu_namelist_at,estimated_value_min,estimated_value_max,decision_timeline,pain_points,interests,decision_maker,budget_range',
            ...
            users: 'id,full_name,email,phone,role,agent_code,team,status,team_id,reporting_to,date_of_birth,employment_type,daily_must_do,created_at,updated_at',
