...
- ⬜ **Reporting — remaining** (backlog, lower traffic, on-click/export not per-render): `build*Details` drill-downs (period-filtered detail tables → server-filtered queries), `renderTargetOverview`, `exportKPIReport`. Note: `getAll('users')` (23 of the 94 call-sites) is NOT a scale risk — 22 rows.
