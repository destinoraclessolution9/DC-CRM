...
- ⬜ **Reporting — remaining** (backlog, lower traffic): the 5 client-side KPIs `calculateKPIs` keeps (`activityHeadcount`, `conversionRate`, `meetUpExistingCount`, `cfHeadcount`, `activeAgents` — multi-table joins, AppDataStore-cached so bounded), the `build*Details` drill-downs (period-filtered detail tables — candidates for server-filtered queries), `renderTargetOverview`, and `exportKPIReport`. Note: `getAll('users')` (23 of the 94 call-sites) is NOT a scale risk — 22 rows.
